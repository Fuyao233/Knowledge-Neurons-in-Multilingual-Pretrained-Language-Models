...
def main():
    ...
    if os.path.exists(args.tmp_data_path):
        with open(args.tmp_data_path, 'r') as f:
            ...
            for key in eval_bag_list_perrel.keys():
                eval_bag_list_perrel[key] = [x for x in eval_bag_list_perrel[key] if len(x) == 6]
            # eval_bag_list_perrel = [x for x in eval_bag_list_perrel if len(x) == 6]
    else:
    ...
    def edit(rel, bag_idx, tgt_ent):
        ...
        for i, (layer, pos) in enumerate(kn_bag):
            model.bert.encoder.layer[layer].output.dense.weight[:, pos].data -= ori_pred_emb * lambda_list_1[i]
            model.bert.encoder.layer[layer].output.dense.weight[:, pos].data += tgt_emb * lambda_list_2[i]
