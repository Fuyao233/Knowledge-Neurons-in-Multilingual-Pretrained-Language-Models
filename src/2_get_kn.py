...
mode_ratio_bag = 0.7
mode_ratio_rel = 0.1
kn_dir = '../results/MultinguakTRExf/kn/'
rlts_dir = '../results/MultinguakTRExf'


def re_filter(metric_triplets):
    # 找到 metric_triplets 中最大响应，再根据最大响应的 threshold_ratio 阈值来找出高于该阈值的神经元
    metric_max = -999
...
def analysis_file(filename, metric='ig_gold'):
    ...
    # rlts_bag_list 记录了对某一关系所有prompt的响应
    # relts_bag 记录了对某一prompt不同提示模板的响应
    for bag_idx, rlts_bag in enumerate(rlts_bag_list):
        ...
        ave_kn_num += len(kn_bag) # 统计每个prompt对应知识神经元综总和
